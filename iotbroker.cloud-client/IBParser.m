...
- (NSMutableData *) encode : (id<IBMessage>) message {
    ...
    if (messageType == IBConnectMessage) {
        ...
        IBUnsuback *suback = (IBUnsuback *)message;
        [buffer appendByte:(messageType << 4)];
        [buffer appendData:[self getBufferByLength:length]];
        if (suback.packetID == 0) {
...
- (id<IBMessage>) decode : (NSMutableData *) buffer {
    ...
    if (messageType == IBConnectMessage) {
        ...
        if ([willQos isValid] != true) {
            @throw [NSException exceptionWithName:@"CONNECT" reason:[NSString stringWithFormat:@"will QoS set to %li", [willQos getValue]] userInfo:nil];
        }
...
//        NSLog(@"Reserved flag       = %@", reservedFlag ? @"yes" : @"no");
        ...
        NSLog(@"packet ID = %li, Topic name = %@, qos = %li, data = %@, is return = %i", packetID, topicName, [qos getValue], [NSString stringWithCString:[data bytes] encoding:NSUTF8StringEncoding], isRetain);
    ...
    if (length.length != [message getLength])
        @throw [NSException exceptionWithName:@"DECODE" reason: [NSString stringWithFormat:@"Invalid length. Encoded: %li, actual: %li", length.length, [message getLength]] userInfo:nil];
