...
- (BOOL)write : (NSData *) data {
    @synchronized(self) {
        ...
        if (self.buffer.length) {
            NSInteger number = [self.stream write:self.buffer.bytes maxLength:self.buffer.length];
            if (number == -1) {
                self.state = IBSocketEncoderErrorState;
                self.error = self.stream.streamError;
                return false;
            } else {
                [self.buffer replaceBytesInRange:NSMakeRange(0, number) withBytes:NULL length:0];
            }
        }
        return true;
    }
